"""
This only works with numpy==1.5.4 due to the nimg.flags.writeable = True
"""
import numpy as np
...
def grey_world(nimg):
    # transpose nimg to shape of (3, x, y)
    nimg = nimg.transpose(2, 0, 1).astype(np.uint32)
    mu_r = np.average(nimg[0])
    mu_g = np.average(nimg[1])
    mu_b = np.average(nimg[2])
    # we only need to modify R, B channels, by coefficient of mu_g/mu_b or mu_g/mu_r
    nimg[0] = np.minimum(nimg[0] * (mu_g / mu_r), 255)
    nimg[2] = np.minimum(nimg[2] * (mu_g / mu_b), 255)
